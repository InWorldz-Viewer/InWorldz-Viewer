...
class PlatformSetup(object):
    generator = None
    build_types = {}
    for t in ('Debug', 'Release', 'ReleaseSSE2', 'RelWithDebInfo', 'ReleaseFast'):
    ...
    def run_vstool(self):
        for build_dir in self.build_dirs():
            ...
            vstool_cmd = (os.path.join('tools','vstool','VSTool.exe') +
                          ' --solution ' +
                          os.path.join(build_dir,'InWorldz.sln') +
                          ' --config ' + self.build_type +
                          ' --startup inworldz-bin')
...
Options:
  ...
  -t | --type=NAME      build type ("Debug", "Release", "ReleaseSSE2", "RelWithDebInfo", or "ReleaseFast")
