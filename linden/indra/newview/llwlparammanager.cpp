...
void LLWLParamManager::loadPreset(const std::string & name,bool propagate)
{
	...
	LL_DEBUGS2("AppInit", "ShaderLoading") << "Loading WindLight sky setting from " << pathName << LL_ENDL; 
	...
	if(!presetsXML)
	{
		...
		LL_DEBUGS2("AppInit", "ShaderLoading")<< "Loading User WindLight sky setting from "  << LL_ENDL; 
		presetsXML.open(pathName.c_str());
	}
