...
bool LLTextureCacheRemoteWorker::doRead()
{
	...
	if (!done && (mState == LOCAL))
	{
		...
		mReadData = new U8[mDataSize];
		S32 bytes_read = LLAPRFile::readEx(local_filename, mReadData, mOffset, mDataSize, mCache->getLocalAPRFilePool());
		if (bytes_read != mDataSize)
		{
 			llwarns << "Error reading file from local cache: " << local_filename
 					<< " Bytes: " << mDataSize << " Offset: " << mOffset
	...
	if (!done && (mState == HEADER))
	{
		...
		S32 bytes_read = LLAPRFile::readEx(mCache->mHeaderDataFileName, mReadData, offset, size, mCache->getLocalAPRFilePool());
		if (bytes_read != size)
		{
	...
	if (!done && (mState == BODY))
	{
		...
		if (filesize && (filesize + TEXTURE_CACHE_ENTRY_SIZE) > mOffset)
		{
			...
			S32 bytes_read = LLAPRFile::readEx(filename, 
											 ...
											 file_offset, file_size, mCache->getLocalAPRFilePool());
...
// 			llinfos << "Writing Body: " << filename << " Bytes: " << file_offset+file_size << llendl;
			S32 bytes_written = LLAPRFile::writeEx(	filename, 
													...
													0, file_size, mCache->getLocalAPRFilePool());
			if (bytes_written <= 0)
			{
				llwarns << "LLTextureCacheWorker: "  << mID
...
void LLTextureCache::purgeCache(ELLPath location)
{
	...
	if (!mReadOnly)
	{
		...
		if(LLFile::isdir(mTexturesDirName))
		{
			...
			if (LLAPRFile::isExist(file_name, getLocalAPRFilePool()))
			{
...
void LLTextureCache::readEntriesHeader()
{
	...
	{
		LLAPRFile::readEx(mHeaderEntriesFileName, (U8*)&mHeaderEntriesInfo, 0, sizeof(EntriesInfo), getLocalAPRFilePool());
...
void LLTextureCache::writeEntriesHeader()
{
	...
	if (!mReadOnly)
	{
		LLAPRFile::writeEx(mHeaderEntriesFileName, (U8*)&mHeaderEntriesInfo, 0, sizeof(EntriesInfo), getLocalAPRFilePool());
	}
}
	...
	if (!mFilesToDelete.empty())
	{
		...
		for (LLTextureCache::purge_map_t::iterator iter = mFilesToDelete.begin(); iter != mFilesToDelete.end(); ) 
		{
			...
			if (mHeaderIDMap.find(curiter->first) == mHeaderIDMap.end())
			{
				...
				//if (LLAPRFile::isExist(filename, getLocalAPRFilePool()))
				//{
