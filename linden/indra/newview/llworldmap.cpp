...
LLSimInfo* LLWorldMap::simInfoFromName(const std::string& sim_name)
{
	...
	if (!sim_name.empty())
	{
		...
		{
			sim_info = (*it).second;
			if (sim_info && (0 == LLStringUtil::compareInsensitive(sim_name, sim_info->mName)) )
 			{
...
{
	return gSavedSettings.getBOOL("UseWebMapTiles") && sGotMapURL && LLWorldMap::getInstance()->mCurrentMap == 0;
}
...
// 			llinfos << "Map sim " << name << " image layer " << agent_flags << " ID " << image_id.getString() << llendl;
		...
		if(LLWorldMap::getInstance()->mIZURLCallback != NULL)
		{
// 				LL_INFOS("World Map") << "New Location " << new_item.mName << LL_ENDL;
			// Server returns definitive capitalization, IZURL might not have that.
			if ((LLStringUtil::compareInsensitive(LLWorldMap::getInstance()->mIZURLRegionName, name)==0)
				|| (LLWorldMap::getInstance()->mIZURLRegionHandle == handle))
			{
				url_callback_t callback = LLWorldMap::getInstance()->mIZURLCallback;

				LLWorldMap::getInstance()->mIZURLCallback = NULL;
				LLWorldMap::getInstance()->mIZURLRegionName.clear();
				LLWorldMap::getInstance()->mIZURLRegionHandle = 0;

				callback(handle, LLWorldMap::getInstance()->mIZURL, image_id, LLWorldMap::getInstance()->mIZURLTeleport);
			}
		}
	}
