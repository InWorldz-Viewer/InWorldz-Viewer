...
// Returns the filename assocated with an id in the local texture list
// Returns "" if the filename could not be found
std::string LocalAssetBrowser::getFilenameFromLocalID(const LLUUID& id)
{
	LocalBitmap* map = GetBitmapUnit(id);
	if (map)
	{
