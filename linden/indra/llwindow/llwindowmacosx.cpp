...
BOOL check_for_card(const char* RENDERER, const char* bad_card)
{
	if (!strnicmp(RENDERER, bad_card, strlen(bad_card)))
	{
		std::string buffer = llformat(
			"Your video card appears to be a %s, which InWorldz does not support.\n"
			"\n"
			"InWorldz requires a video card with 32 Mb of memory or more, as well as\n"
			...
			"inworldz.com support section or e-mail technical support\n"
			"\n"
			"You can try to run InWorldz, but it will probably crash or run\n"
	...
	: LLWindow(fullscreen, flags)
{
	...
	if(createContext(x, y, width, height, 32, fullscreen, disable_vsync))
	{
		...
		{
			setupFailure(
				"InWorldz is unable to run because your video card drivers\n"
...
BOOL LLWindowMacOSX::createContext(int x, int y, int width, int height, int bits, BOOL fullscreen, BOOL disable_vsync)
{
	...
	if (colorBits < 32)
	{
		close();
		setupFailure(
			"InWorldz requires True Color (32-bit) to run in a window.\n"
			"Please go to Control Panels -> Display -> Settings and\n"
			"set the screen to 32-bit color.\n"
			"Alternately, if you choose to run fullscreen, InWorldz\n"
	...
	if (alphaBits < 8)
	{
		close();
		setupFailure(
			"InWorldz is unable to run because it can't get an 8 bit alpha\n"
